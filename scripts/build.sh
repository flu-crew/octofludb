...
# log "Loading GISAID metadata"
# parallel --citation "d79 load_gis {} > ttl/{/}.ttl" ::: STATIC/epiflu/h[13]/*xls

# log "Loading GISAID sequence data"
# parallel "sed 's/>\([0-9]*\)/>EPI\1/' {} | d79 load_fasta > ttl/{/}.ttl" ::: STATIC/epiflu/h[13]/*fasta

# file="${DATADIR}/antiserum/antiserum_strain_names"
# tag="antiserum"
# log "Loading antisera strain names from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl
#
# file="${DATADIR}/antiserum/antigen_strain_names"
# tag="antigen"
# log "Loading antigen strain names from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl
#
# file="STATIC/CDC_CVV/isolate_ids.txt"
# tag="cdc_cvv"
# log "Loading CDC_CVV strains from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl
#
# file="${DATADIR}/vaccine/isolate_ids.txt"
# tag="vaccine"
# log "Loading strain recommended by WHO for vaccines from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl
#
# file="${DATADIR}/variants/isolate_ids.txt"
# tag="variants"
# log "Loading variants from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl


# file="${DATADIR}/octoflu-references/segment-ids.txt"
# tag="octoflu-reference"
# log "Loading swine octoflu references from '$file' as tag '$tag'"
# time d79 tag "$file" --tag="$tag" > ttl/$tag.ttl
